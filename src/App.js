...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (countdown === 0) {
      ...
      if (video && canvas && video.videoWidth > 0) {
        ...
        setError("Camera wasn't ready. Let's try again!");
  ...
  const generateImage = useCallback(async (guestDataUrl) => {
    ...
    try {
      ...
      setError(err.message || "Something went wrong creating your portrait. Let's try again!");
      setStep(STEPS.WELCOME);
    }
  }, []);
    ...
    try {
      if (result.startsWith("data:")) {
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({ title: "Photo with " + COUPLE_NAMES, files: [file] });
          return;
        }
      }
      if (navigator.share) {
        await navigator.share({ title: "Photo with " + COUPLE_NAMES, text: "Check out my photo with " + COUPLE_NAMES + "!" });
      }
    } catch (e) {
      // user cancelled
    }
  ...
  const Rings = ({ size = 48 }) => (
    <svg width={size} height={size * 0.65} viewBox="0 0 80 52" fill="none" style={{ opacity: 0.45 }}>
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "linear-gradient(170deg, #141110 0%, #2a1f18 40%, #1a1410 100%)",
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
        {Array.from({ length: 14 }).map((_, i) => (
          <div key={i} style={{
            position: "absolute",
            width: Math.random() * 8 + 3, height: Math.random() * 8 + 3,
            borderRadius: "50%",
            background: ["#c9956b","#d4a574","#e8c4a0","#b8835a","#f0d9b5"][i % 5],
      ...
      <div style={{
        ...
        {step === STEPS.WELCOME && (
          <div style={{
            ...
            textAlign: "center", gap: 24, animation: "fadeUp .8s ease-out",
          }}>
            <Rings size={56} />
            <div style={{
              fontSize: 11, letterSpacing: 5,
              fontFamily: "'Josefin Sans', sans-serif", fontWeight: 200,
              textTransform: "uppercase", color: "#c9956b",
            }}>The wedding of</div>
            <h1 style={{
              fontSize: "clamp(34px, 7vw, 54px)",
              fontWeight: 300, fontStyle: "italic",
              lineHeight: 1.15, letterSpacing: 1,
              background: "linear-gradient(135deg, #f0e6d8, #c9956b, #f0e6d8)",
            ...
            <div style={{
              ...
              fontWeight: 200, letterSpacing: 2, opacity: 0.5,
            }}>{WEDDING_DATE}</div>
            <div style={{ width: 80, height: 1, background: "linear-gradient(90deg, transparent, #c9956b, transparent)" }} />
            <p style={{
              fontSize: 17, fontStyle: "italic",
              lineHeight: 1.7, opacity: 0.6, maxWidth: 360,
            }}>
              Take a selfie and we'll create a beautiful photo of you with the happy couple
            </p>
            <button className="btn" onClick={openCamera} style={{ marginTop: 8 }}>
              Take My Photo
            </button>
            {error && (
              <div style={{
                ...
                fontWeight: 300, color: "#e8a090", maxWidth: 380,
              }}>{error}</div>
            )}
          </div>
        )}
        ...
        {step === STEPS.CAMERA && (
          <div style={{
            flex: 1, display: "flex", flexDirection: "column",
            alignItems: "center", justifyContent: "center",
            gap: 24, animation: "fadeUp .5s ease-out",
          }}>
            <p style={{ fontSize: 18, fontStyle: "italic", opacity: 0.6, textAlign: "center" }}>
              Strike a pose with {COUPLE_NAMES}!
            </p>
            <div style={{
              position: "relative", width: "100%",
              aspectRatio: "3/4", maxHeight: "55vh",
              ...
              {[["top","left"],["top","right"],["bottom","left"],["bottom","right"]].map(([v,h]) => (
                <div key={v+h} style={{
                  position:"absolute", [v]:14, [h]:14, width:24, height:24,
                  [`border${v[0].toUpperCase()+v.slice(1)}`]:"1.5px solid rgba(201,149,107,.5)",
                  [`border${h[0].toUpperCase()+h.slice(1)}`]:"1.5px solid rgba(201,149,107,.5)",
                }} />
              ))}
              {countdown !== null && countdown > 0 && (
                <div style={{
                  ...
                  <span key={countdown} style={{
                    fontSize:110, fontWeight:300, color:"#f0e6d8",
              ...
              {!cameraReady && (
                <div style={{
                  ...
                  <div style={{
                    width: 36, height: 36,
                    border: "2px solid rgba(201,149,107,.2)",
                    borderTopColor: "#c9956b", borderRadius: "50%",
            ...
            <button
              ...
              style={{ opacity: (countdown !== null || !cameraReady) ? 0.5 : 1 }}
            >
              {!cameraReady ? "Starting camera…" : countdown !== null ? "Hold still…" : "Capture"}
            </button>
          </div>
        )}
        ...
        {step === STEPS.GENERATING && (
          <div style={{
            flex: 1, display: "flex", flexDirection: "column",
            alignItems: "center", justifyContent: "center",
            gap: 32, animation: "fadeUp .5s ease-out",
          }}>
            {selfie && (
              <div style={{
            ...
            <div style={{ textAlign: "center" }}>
              <p style={{
                fontSize: 22, fontStyle: "italic", marginBottom: 10,
                animation: "breathe 3s ease-in-out infinite",
              }}>{LOADING_MESSAGES[loadingMsg]}</p>
              <p style={{
                fontSize: 12, fontFamily: "'Josefin Sans', sans-serif",
                fontWeight: 200, letterSpacing: 1.5, opacity: 0.35, textTransform: "uppercase",
              }}>Creating your photo with {COUPLE_NAMES}</p>
            </div>
          </div>
        )}
        ...
        {step === STEPS.RESULT && result && (
          <div style={{
            flex: 1, display: "flex", flexDirection: "column",
            alignItems: "center", justifyContent: "center",
            gap: 24, padding: "32px 0", animation: "fadeUp .6s ease-out",
          }}>
            <Rings size={40} />
            <p style={{ fontSize: 24, fontStyle: "italic", textAlign: "center" }}>
              A moment to remember!
            </p>
            <div style={{
              width: "100%", borderRadius: 6, overflow: "hidden",
              border: "1px solid rgba(201,149,107,.35)",
              boxShadow: "0 24px 80px rgba(201,149,107,.12), 0 32px 64px rgba(0,0,0,.5)",
              animation: "resultReveal .8s ease-out",
            }}>
              <img src={result} alt="Wedding Portrait" style={{ width: "100%", display: "block" }} />
            </div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", marginTop: 4 }}>
              <button className="btn btn-outline" onClick={startOver}>Take Another</button>
              {typeof navigator !== "undefined" && navigator.share && (
                <button className="btn btn-outline" onClick={sharePhoto}>Share</button>
              )}
              <a
                href={result}
        ...
        <div style={{
          ...
          fontWeight: 200, letterSpacing: 2.5,
          textTransform: "uppercase", opacity: 0.2,
        }}>{COUPLE_NAMES} · Wedding Photo Booth</div>
      </div>
    </div>
  );
}
